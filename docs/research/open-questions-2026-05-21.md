...
**UPDATE 2026-05-21 Option C MERGER**: Owner audit found existing Codex Directive E application on 754 grammar items is template injection (generic filler phrases like "Nếu câu khó dịch, hãy tách mẫu ra khỏi từ vựng trước" applicable to any pattern). Owner chose Option C: merge Directive E quality redo INTO Phase G Tier-1 hand-crafted authoring. Same Top-200 scope, same quality bar, 1-pass authoring. Tier-2 (~21K) keeps existing Directive E template (no regen in Sprint 4). Phase G now has TWO tracks per Tier-1 item: Track A (exercise templates) + Track B (Directive E real Dr. Linh-Phan-Trần voice with etymology, Hán-Việt bridge, pattern-specific Human Moment, cross-links). Added `tool/qa/validate_directive_e_quality.js` validator: substitution test + banned phrase list + pattern reference + etymology length + cross-links non-empty. Items failing validator → re-author, do not commit.
...
**Owner answer**: OPTION (c) PROGRESSIVE DISCLOSURE 2026-05-22 — Render grammar Directive E in a single scrollable detail page with progressive disclosure. Above-the-fold (always visible): `form` (Cấu trúc, prominent), `meaning` (Ý nghĩa), `usage` (1-line summary or first 2 sentences). Below-the-fold (collapsed under "Xem thêm" / "Tìm hiểu sâu hơn" expander, default collapsed): `etymology` (Gốc rễ + Hán-Việt bridge if present), `humanMoment` (Lưu ý từ Dr. Linh), `crossLinks` (Mẫu liên quan with contrast labels), `fallbackReference` source attribution. Examples (≥10) shown in their own collapsible section "Ví dụ" or always visible if ≤5. Smooth slide-down animation on expand; ARIA `aria-expanded` for accessibility. Mobile: full-width sections, large tap targets (≥44px). Apply to Tier-1 (rich content) AND Tier-2 (template content) — progressive disclosure works for both, avoids overwhelming beginners while preserving depth for advanced learners. Sticky "Làm bài tập" CTA at bottom of viewport. Implementation lives in `lib/features/grammar/widgets/grammar_directive_e_section.dart` (new) using foundation primitives (`AppCard`, `AppSection`, `AppDivider`, expand control built from `AppButton` ghost variant). Update routes and grammar detail screen to wire the new widget. Add unit test + Playwright visual regression at 4 viewports.
