...
- Verified so far: JSON parse passed, no false approval/draft tags remain in lesson 09, coverage audit reduced N2 incomplete current entries from `136` to `128`, and focused DB/reachability/taxonomy/upper-JLPT tests passed. Wide gates and live proof pending.
