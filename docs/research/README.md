...
- Kanji expansion status update: QA-B-002 now has a reproducible KANJIDIC2 old-JLPT coverage audit. Baseline current unique Kanji is `638` vs KANJIDIC2 old-JLPT unique `2230`; missing source Kanji are N5 `33`, N4 `157`, N2 `654`, N1 `1168`. KANJIDIC2 has no modern N3 tier, so a separate modern JLPT level map is required before generation. N3 source verification is complete with incomplete current entries at `0`; N2 lessons 01-05 are source-verified/live-verified and N2 incomplete current entries are reduced to `160`. QA-A-016 changed non-fingerprinted Flutter shell/content assets to `Cache-Control: no-cache` after a one-day `main.dart.js` cache masked the N2 lesson-03 DB sentinel on live proof. Heavy runtime files (`sqlite3.wasm`, `drift_worker.js`) remain on long cache. N2/N1 coverage debt remains.
