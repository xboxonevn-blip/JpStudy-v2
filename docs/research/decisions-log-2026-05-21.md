...
## DECISION-158 - Separate interpretive lens, best-choice limit, necessary experience, extreme concession, and removed component
- Date: 2026-05-22
- Decision: In Directive E batch 058, re-author `〜とみると`, `Noun に限る`, `Verb てこそ`, `どんなに～うが`, and `Noun ぬいた Noun`; repair one malformed `とみると` example in lesson 52.
- Rationale: The rows now distinguish viewing through a chosen lens from passive assessment, limiting the best choice to N, necessary action emphasized by `こそ`, extreme concession through volitional/かろう/だろう + が, and a noun left after a component is pulled out with `抜く`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_52:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_52:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_53:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_53:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_53:003`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `370/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-159 - Separate repeated trigger, silent-message adverb, through-to-end connector, repeated action pair, and silent-message noun modifier
- Date: 2026-05-22
- Decision: In Directive E batch 059, re-author `Verb ては`, `〜と言わんばかりに`, `Verb ぬいて`, `Verb ては Verb`, and `〜と言わんばかりの Noun`; rename the lesson-54 `ぬいて` item from noun-exclusion to verb-completion to match its examples.
- Rationale: The rows now distinguish repeated trigger cycles, body-language-as-speech adverbial use, `抜く` as pushing through to the end, explicit repeated action pairs, and the noun-modifying silent-message form.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_54:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_54:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_54:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_55:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_55:002`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `375/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-160 - Separate completion-through-resistance, costly means, fixed-state background, formal dependence, and proof-by-action
- Date: 2026-05-22
- Decision: In Directive E batch 060, re-author `〜ぬく`, `Verb てまでも`, `〜ながらに`, `Noun+のいかんだ`, and `Verb てみせる`; repair unnatural examples for `ぬく`, `のいかん`, and `てみせる`.
- Rationale: The rows now distinguish completion through resistance from simple completion, a costly means that reaches an extreme `まで`, fixed/state-background phrases with `ながらに`, formal dependence through `如何`, and action used as visible proof through `見せる`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_55:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_56:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_56:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_56:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_57:001`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `380/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-161 - Separate noun-modifying simultaneity, conditional dependence, unceasing emotion, concessive state, and regardless-of-condition
- Date: 2026-05-22
- Decision: In Directive E batch 061, re-author `～ながらの Noun`, `Noun のいかんでは`, `Verb てやまない`, `～ながらも`, and `Noun のいかんにかかわらず`; repair one invalid `ながらも` example.
- Rationale: The rows now distinguish `ながらの` as a noun modifier, `のいかんでは` as a formal condition branch, `てやまない` as unceasing emotion from `止む`, `ながらも` as concession through `も`, and `のいかんにかかわらず` as denying dependence on how N turns out.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_57:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_57:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_58:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_58:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_58:003`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `385/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-162 - Separate inevitable reaction, double-negative reservation, formal contingent result, indispensable absence, and tolerated-vs-rejected contrast
- Date: 2026-05-22
- Decision: In Directive E batch 062, re-author `Verb ないではおかない`, `～なくはない`, `Noun のいかんによっては`, `Noun なしには～ない`, and `Noun ならいざ知らず`; repair weak lesson-59 examples for inevitable reaction and double-negative reservation.
- Rationale: The rows now distinguish not-leaving-until-V through `おく`, reserved affirmation through double negation, contingent formal outcome via `如何` + `によっては`, indispensable absence via `なしには`, and the old-style `いざ知らず` contrast between tolerable and rejected cases.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_59:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_59:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_59:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_6:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_6:002`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `390/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-163 - Separate uniqueness, open alternatives, minimum acceptable amount, layered accumulation, and formal circumstance frame
- Date: 2026-05-22
- Decision: In Directive E batch 063, re-author `Noun + ならでは`, `Noun なり Noun なり`, `Noun なりとも`, `Noun に Noun を重ねて`, and `Noun にあっては`.
- Rationale: The rows now distinguish uniqueness through `ならでは`, open same-kind alternatives through repeated `なり`, minimal acceptable value through `なりとも`, layered accumulation through `重ねる`, and formal circumstance framing through `にあっては`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_6:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_6:004`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_6:005`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_6:006`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_6:007`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `395/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-164 - Separate unbecoming role violation, formal threshold state, unresolved obligation, soft double-negative possibility, and regardless-of-how condition
- Date: 2026-05-22
- Decision: In Directive E batch 064, re-author `Noun にあるまじき Noun`, `Noun にして`, `Verb ないではすまない`, `～なくもない`, and `Noun のいかんによらず`; repair malformed `なくもない` examples and tighten `すまない`/policy examples.
- Rationale: The rows now distinguish role-violation reproach through `あるまじき`, formal state/threshold evaluation via `にして`, unresolved duty through `済む`, soft double-negative possibility through `なくもない`, and non-dependence on how N turns out via `のいかんによらず`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_6:008`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_6:009`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_60:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_60:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_60:003`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `400/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-165 - Separate not-quite-threshold concession, fit-to-own-condition, literary comparison, hopeful negative question, and excessive-action boundary
- Date: 2026-05-22
- Decision: In Directive E batch 065, re-author `Verb ないまでも`, `～なら～なりに`, `Noun のごとき Noun`, `Verb ないものだろうか`, and `～には及ばない`; repair one `ないまでも` example that used `にしても` instead.
- Rationale: The rows now distinguish not reaching an ideal threshold, acting according to one's own condition, literary comparison through `如き`, hopeful self-questioning through `ものだろうか`, and the boundary where an action is unnecessary through `及ぶ`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_61:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_61:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_61:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_62:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_62:002`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `405/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-166 - Separate known-character prediction, reserved possibility, unbearable threshold, bad tendency, and missing-noun modifier
- Date: 2026-05-22
- Decision: In Directive E batch 066, re-author `Noun のことだから`, `Verb ないものでもない`, `～に堪えない`, `Verb/Noun の嫌いがある`, and `Noun のない Noun`; align `嫌いがある` as tendency rather than dislike and `のない` as noun absence rather than verb negation.
- Rationale: The rows now distinguish prediction from known character, guarded possibility through double negation, the broken endurance threshold of `堪`, formal bad tendency through `嫌いがある`, and a noun modifier meaning “without N”.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_62:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_63:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_63:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_63:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_64:001`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `410/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-167 - Separate worthiness, face constraint, endless listing, endurance force, and emotional extreme
- Date: 2026-05-22
- Decision: In Directive E batch 067, re-author `～に堪える`, `Noun の手前 / Verbた手前`, `Verb ば きりがない`, `～に耐える`, and `Noun の極み`; align `手前` examples to face/social constraint and improve `極み` examples.
- Rationale: The rows now distinguish worthiness through `堪える`, social face through `手前`, endless chains through `きりがない`, physical/mental endurance through `耐える`, and emotional extremes through `極み`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_64:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_64:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_65:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_65:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_65:003`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `415/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-168 - Separate skipped minimum action, reached result, formal extreme, colloquial rejection, and optimal choice
- Date: 2026-05-22
- Decision: In Directive E batch 068, re-author `Verb もしないで`, `～に至った`, `Noun の至り`, `Verb やしない`, and `～に越したことはない`; repair examples for skipped exam study, formal `の至り`, and adjective/noun attachment before `に越したことはない`.
- Rationale: The rows now distinguish skipped minimum action through `も` + `しないで`, reached result through `至る`, formal emotional extreme through `至り`, colloquial strong rejection through `やしない`, and optimal choice through `越す` + `ことはない`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_66:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_66:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_66:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_67:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_67:002`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `420/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-169 - Separate accepted-vs-surprising contrast, manner target, formal easy inference, minimum-level emphasis, and do-or-not indecision
- Date: 2026-05-22
- Decision: In Directive E batch 069, re-author `Noun はいざ知らず`, `Verb よう`, `～に難くない`, `Noun はおろか～すら`, and `Verb ようか Verbるまいか`; repair one `はおろか～すら` example so the second noun explicitly uses `すら`.
- Rationale: The rows now distinguish accepted-vs-surprising contrast through `いざ知らず`, manner/target through `様` + `に`, formal easy inference through `難くない`, minimum-level emphasis through `すら/さえ`, and indecision through volitional `よう` versus negative-volitional `まい`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_67:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_68:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_68:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_68:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_69:001`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `425/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-170 - Separate sole remaining value, extended endpoint, threshold realization, extreme target marking, and range expansion
- Date: 2026-05-22
- Decision: In Directive E batch 070, re-author `～のは Noun ぐらいのものだ`, `Noun はおろか～まで`, `Noun にして初めて`, `Noun にすら`, and `Noun にとどまらず～も`.
- Rationale: The rows now distinguish sole remaining value through `ぐらいのものだ`, extended endpoint through `まで`, threshold realization through `にして初めて`, extreme target marking through `にすら`, and range expansion through `にとどまらず`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_69:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_69:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_7:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_7:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_7:003`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `430/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-171 - Separate strong contrast, greater-than benchmark, no-dependence criteria, pioneering order, and basis-fit modification
- Date: 2026-05-22
- Decision: In Directive E batch 071, re-author `Noun にひきかえ Noun は`, `Noun にもまして`, `Noun によらず`, `Noun に先駆けて`, and `Noun に即した Noun`.
- Rationale: The rows now distinguish strong contrast through `引き換え`, benchmark exceeding through `増す`, criterion independence through `依らず`, pioneering order through `先駆けて`, and basis-fit noun modification through `即した`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_7:004`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_7:005`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_7:006`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_7:007`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_7:008`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `435/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-172 - Separate basis-bound actions, viewpoint attribution, do-or-not invariance, regretful counterfactuals, and no-method impossibility
- Date: 2026-05-22
- Decision: In Directive E batch 072, re-author `Noun に即して Verb`, `Noun に言わせれば`, `Verb ようが Verbるまいが`, `～ば～ものを`, and `Verb ようがない`; repair one `ようが...まいが` example that used adjectives and two `に言わせれば` attribution examples.
- Rationale: The rows now distinguish basis-bound action through `即して`, viewpoint attribution through causative `言わせる`, invariance across doing/not-doing through `ようが...まいが`, regretful counterfactuals through `ものを`, and no-method impossibility through `ようがない`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_7:009`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_7:010`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_70:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_70:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_71:001`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `440/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-173 - Separate natural appearance, do-or-not condition listing, becoming appearance, blocked intention, and pretended posture
- Date: 2026-05-22
- Decision: In Directive E batch 073, re-author `～びた`, `Verb ようと Verbるまいと`, `～びる`, `Verb ようにも`, and `～ぶった`; repair a malformed `～ぶった` example.
- Rationale: The rows now distinguish natural adjectival appearance through `びた`, listed do-or-not invariance through `ようと...まいと`, becoming appearance through `びる`, blocked intention through `ようにも`, and pretended posture through `ぶった`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_71:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_72:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_72:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_73:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_73:002`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `445/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-174 - Separate blocked potential, te-form posturing, no-method emphasis, return-after-interval, and as-is flow
- Date: 2026-05-22
- Decision: In Directive E batch 074, re-author `Verb ようにも Verbれない`, `～ぶって`, `Verb ようもない`, `～ぶり`, and `Verbる がままに`; repair malformed `～ぶって` and weak `がままに` examples.
- Rationale: The rows now distinguish blocked potential through `ようにも...れない`, te-form posturing through `ぶって`, emphatic no-method through `ようもない`, return after an interval through `ぶり`, and as-is natural flow through `がままに`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_74:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_74:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_75:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_75:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_76:001`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `450/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-175 - Separate posturing verb, immediate succession, endpoint assertion, avoidant preference, and strong label denial
- Date: 2026-05-22
- Decision: In Directive E batch 075, re-author `～ぶる`, `Verbる が早いか`, `～までだ`, `Verbる くらいなら`, and `～もなんでもない`; repair malformed examples for `が早いか` and `もなんでもない`.
- Rationale: The rows now distinguish posturing through `ぶる`, immediate succession through `が早いか`, endpoint assertion through `までだ`, avoidant preference through `くらいなら`, and strong label denial through `もなんでもない`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_76:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_77:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_77:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_78:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_78:002`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `455/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-176 - Separate without-action omission, assumption premise, non-limited scope, soft scope denial, and best-choice limit
- Date: 2026-05-22
- Decision: In Directive E batch 076, re-author `Verbる ことなしに`, `～ものとして`, `Noun に限ったことではない`, `Noun に限ったことでもない`, and `Noun/Verb に限る`; repair weak `ことなしに` and `ものとして` examples and broaden `に限る` structure.
- Rationale: The rows now distinguish omitted procedural action through `ことなしに`, assumption premise through `ものとして`, non-limited scope through `に限ったことではない`, softer scope denial through `でもない`, and best-choice limiting through `に限る`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_79:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_79:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_8:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_8:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_8:003`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `460/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-177 - Separate removed modifier, exclusion frame, endurance completion, formal dependency, and conditional branch
- Date: 2026-05-22
- Decision: In Directive E batch 077, re-author `Noun1 + をぬいた + Noun2`, `Noun + をぬいて`, `Verb-stem + ぬく`, `Noun + のいかんで`, and `Noun + のいかんでは`; repair malformed examples for `ぬいて`, unnatural `ぬく` verbs, and invalid `のいかん` clauses.
- Rationale: The rows now distinguish removed-result noun modification through `をぬいた`, exclusion framing through `をぬいて`, endurance-to-completion through suffix `ぬく`, formal result dependency through `のいかんで`, and branch-condition consequence through `のいかんでは`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_8:004`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_8:005`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_8:006`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_8:007`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_8:008`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `465/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-178 - Separate irrespective outcome, possible branch, prevention target, literary purpose, and endless condition
- Date: 2026-05-22
- Decision: In Directive E batch 078, re-author `Noun のいかんにかかわらず`, `Noun のいかんによっては`, `Verbる ことのないように`, `～んがために`, and `Verbる ときりがない`; repair weak examples for `のいかんによっては`, prevention, literary-purpose, and endless-listing patterns.
- Rationale: The rows now distinguish invariant outcome through `にかかわらず`, possible exceptional branch through `によっては`, prevention target through `ことのないように`, literary determined purpose through `んがために`, and no-end condition through `ときりがない`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_8:009`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_8:010`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_80:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_80:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_81:001`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `470/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-179 - Separate near-action gesture, absentminded action, formal no-obstacle permission, unintentional adverbial flow, and formal occasion
- Date: 2026-05-22
- Decision: In Directive E batch 079, re-author `～んばかりに`, `Verbる ともなく Verb`, `～差し支えない`, `Verbる ともなしに Verb`, and `～折に`; correct `差し支えない` structure and repair weak examples for all five patterns.
- Rationale: The rows now distinguish near-action gesture through `んばかりに`, absentminded repeated action through `ともなく`, formal no-obstacle permission through `差し支えない`, adverbial unintentional flow through `ともなしに`, and formal occasion marking through `折に`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_81:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_82:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_82:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_83:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_83:002`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `475/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-180 - Separate immediate succession, extreme judgment, range expansion, reached extreme, and no-need reaction
- Date: 2026-05-23
- Decision: In Directive E batch 080, re-author `Verbる なり`, `～極まりない`, `Verbる にとどまらず～も`, `～極まる`, and `Verbる にはあたらない`; repair examples for `極まりない`, `にとどまらず`, `極まる`, and `にはあたらない`.
- Rationale: The rows now distinguish immediate one-off succession through `なり`, formal extreme judgment through `極まりない`, scope expansion through `にとどまらず`, reaching an extreme through `極まる`, and judging a reaction unnecessary through `にはあたらない`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_84:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_84:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_85:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_85:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_86:001`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `480/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-181 - Separate insufficiency, constrained action, worthiness, blocked volition, and emotional limit
- Date: 2026-05-23
- Decision: In Directive E batch 081, re-author `～足りない`, `Verbる にも`, `～足る Noun`, `Verb-volitional + にも + Verb-potential negative`, and `～限りだ`; repair examples for `Verbる にも`, `～足る Noun`, blocked-volition, and `～限りだ`.
- Rationale: The rows now distinguish insufficiency through negative `足りる`, constrained action through `にも`, formal worthiness through `に足る`, blocked volition through volitional plus potential-negative pairing, and emotional maximum through `限り`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_86:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_87:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_87:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_88:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_88:002`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `485/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-182 - Separate invariant condition, literary simile, known-character prediction, critical tendency, and face constraint
- Date: 2026-05-23
- Decision: In Directive E batch 082, re-author `Noun のいかんによらず`, `Noun のごとき Noun`, `Noun のことだから`, `Noun の嫌いがある`, and `Noun の手前`; repair examples for literary simile, tendency-not-dislike, and face-pressure `手前`.
- Rationale: The rows now distinguish invariant formal conditions through `如何` + `よらず`, archaic simile through `如き`, predictions from known character through `のことだから`, formal negative tendency through `嫌いがある`, and social face constraints through `手前`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_9:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_9:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_9:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_9:004`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_9:005`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `490/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-183 - Separate emotional apex, formal extremity, accepted-vs-surprising contrast, minimum-level emphasis, and range expansion
- Date: 2026-05-23
- Decision: In Directive E batch 083, re-author `Noun の極み`, `Noun の至り`, `Noun はいざ知らず`, `Noun はおろか～すら`, and `Noun はおろか～まで`; repair examples for emotional apex, minimum-level emphasis, and range expansion.
- Rationale: The rows now distinguish the apex metaphor of `極み`, formal reached extremity of `至り`, accepted-vs-surprising contrast through `はいざ知らず`, minimum-level emphasis through `すら/さえ`, and far-reach expansion through `まで`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_9:006`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_9:007`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_9:008`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_9:009`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n1:grammar_n1_9:010`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `495/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-184 - Separate alternatives, additive burden, topic shift, emphatic addition, and logical consequence
- Date: 2026-05-23
- Decision: In Directive E batch 084, re-author N2 connector rows `A あるいは B`, `A。おまけに B。`, `A。さて B。`, `A。しかも B。`, and `A。したがって B。`; repair `あるいは` examples to use natural same-rank alternatives.
- Rationale: The rows now distinguish formal alternatives through `或いは`, add-on burden through `おまけ`, discourse topic shift through `さて`, emphatic supporting addition through `しかも`, and logical consequence through `従う`/`したがって`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n2:grammar_n2_1:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n2:grammar_n2_1:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n2:grammar_n2_1:003`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n2:grammar_n2_1:004`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n2:grammar_n2_1:005`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `500/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-185 - Separate equivalence, natural consequence, reputation-fit, lesser-bad, and immediate result point
- Date: 2026-05-23
- Decision: In Directive E batch 085, re-author `A すなわち B`, `A。すると B。`, `～だけあって`, `～だけましだ`, and `～たところ`; repair `だけあって` and `だけましだ` examples for natural attachment and lesser-bad context.
- Rationale: The rows now distinguish equivalence through `即ち`, natural consequence through `すると`, expectation fit through `だけあって`, a limited saving point through `だけましだ`, and an immediate post-action result point through `ところ`.
- Verification: `node tool/qa/validate_directive_e_quality.js --item-id grammar:n2:grammar_n2_1:006`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n2:grammar_n2_1:007`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n2:grammar_n2_10:001`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n2:grammar_n2_10:002`; `node tool/qa/validate_directive_e_quality.js --item-id grammar:n2:grammar_n2_10:003`; `node tool/qa/validate_directive_e_quality.js --rank-json assets/data/content/exercises/top_200_frequency_rank.json`; global validator progress `505/1123`; `node --test test/tool/research/directive_e_quality_validator_test.js`; `flutter test test/data/upper_jlpt_content_integrity_test.dart test/data/utils/grammar_example_quality_test.dart test/features/grammar/grammar_detail_screen_test.dart --reporter expanded`; `git diff --check`.

## DECISION-186 - Reject Phase E example-sentence template fillers globally
- Date: 2026-05-23
- Decision: Treat Phase E vocab `example_sentences[]` with the same anti-template bar as Directive E; strengthen `validate_example_quality.js` to reject Japanese/Vietnamese "use this word" fillers plus article-placeholder frames, scan all content JSON/Markdown for those strings, regenerate `examples_corpus.json`, and rewire all vocab files from Tatoeba-first rows or JpStudy-authored contextual fallback.
- Rationale: Audit #25 showed `examples_corpus.json` was no longer allowed to be a template source. The corpus now prefers Tatoeba CC-BY 2.0 rows, accepts owner-local textbook cache rows when supplied, and otherwise emits contextual authored examples with source/attribution metadata instead of substitution-safe filler.
- Verification: `node --test test/tool/qa/validate_example_quality_test.js test/tool/migration/wire_example_sentences_test.js`; `node tool/qa/validate_example_quality.js`; `node tool/migration/wire_example_sentences.js --validate-only`; `node tool/research/wire_example_sentences.js --validate-only`; hard banned-phrase scan over `assets/data/content/examples_corpus.json` and `assets/data/content/vocab`; corpus source mix `19364 total / 5786 Tatoeba / 13578 authored`; `flutter analyze lib/data/db/content_database.dart lib/data/repositories assets/data/content/vocab tool/migration/wire_example_sentences.js tool/qa/validate_example_quality.js`; `flutter test test/data/db/content_database_lazy_seed_test.dart test/data/repositories/lesson_repository_test.dart --reporter expanded`; `flutter build web`; local Playwright verify on `http://127.0.0.1:52725/?qa=examples25b#/lesson/1?level=N5` showed Minna N5 Bài 1 `私` context card as `私の番？ / Đến lượt tôi chưa?`; `git diff --check`.

## DECISION-187 - Use runtime Web Speech for P0 listening audio
- Date: 2026-05-24
- Decision: Implement the P0 listening/audio blocker with a small `TtsService` abstraction and Web Speech API `speechSynthesis` on web, with no bundled audio assets for now. Non-web gets an explicit unavailable result instead of silent buttons.
- Rationale: Web Speech gives immediate Japanese TTS coverage for flashcards, vocab, grammar examples, lesson listening mode, and mock-exam listening without adding package weight or pre-generated audio management. Runtime TTS also keeps empty `audio_url` fields acceptable because the app can read each item's kana/term/example text directly.
- Verification: `flutter analyze lib test`; focused P0 Flutter test bundle; `flutter build web --release`; Playwright desktop listening verified `speechSynthesis.speak` text `しゃいん`, lang `ja-JP`, rate `0.9`; Playwright mock exam listening verified `speechSynthesis.speak` text `しゅっちょうします`; Playwright mobile verified More sheet content plus overlay dismissal after route change. Screenshots archived under `docs/research/qa-live-2026-05-25-p0-*.png`.
