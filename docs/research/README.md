...
- Kanji expansion status update: QA-B-002 now has a reproducible KANJIDIC2 old-JLPT coverage audit. Baseline current unique Kanji is `638` vs KANJIDIC2 old-JLPT unique `2230`; missing source Kanji are N5 `33`, N4 `157`, N2 `654`, N1 `1168`. KANJIDIC2 has no modern N3 tier, so a separate modern JLPT level map is required before generation. N3 source verification is complete with incomplete current entries at `0`; N2 lessons 01-21 are source-verified, live-verified, and N2 incomplete current entries are reduced to `32`; lesson 21 live proof fetched deployed `補` metadata with `Bổ (bổ sung; bù đắp; hỗ trợ)`, Hán-Việt `Bổ`, on `ホ`, kun `おぎな.う`, and stroke count `12`. QA-A-016 found that fixed app-shell caching can mask seed sentinels on live proof; current deploy verification uses cache-disabled/no-store checks before treating production state as current. N2/N1 coverage debt remains.
