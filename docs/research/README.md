...
- Kanji expansion status update: QA-B-002 now has a reproducible KANJIDIC2 old-JLPT coverage audit. Baseline current unique Kanji is `638` vs KANJIDIC2 old-JLPT unique `2230`; missing source Kanji are N5 `33`, N4 `157`, N2 `654`, N1 `1168`. KANJIDIC2 has no modern N3 tier, so a separate modern JLPT level map is required before generation. N3 source verification is complete with incomplete current entries at `0`; N2 lessons 01-25 are source-verified, live-verified, and N2 incomplete current entries are `0`; N1 lessons 1-22 are source-verified/live-verified, reducing N1 incomplete current entries to `24`; latest live proof rendered the real VI/N1 Kanji UI with normal browser cache: search `議` showed `Hán-Việt Nghị`, detail modal showed `Nghị (bàn luận; nghị sự; ý kiến)`, search `別` showed `Hán-Việt Biệt`, and page-context fetch returned content/shell `Cache-Control: no-cache`. QA-A-017 restored `no-cache` for app-shell/content assets; deploy verification now includes normal-cache header proof and rendered UI proof, with CDP cache-disabled reserved only as a diagnostic control. N1 coverage debt remains.
