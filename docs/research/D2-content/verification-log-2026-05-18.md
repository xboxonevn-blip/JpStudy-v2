...
## Kanji N1 Lessons 23-25 Completeness Batch

Owner loop update continues: lessons 23-25 close the remaining current N1 completeness debt as one source-verified batch. Gate/deploy/live proof is run once for the batch, with normal browser cache and rendered app UI required.

Sources consulted:

- KANJIDIC2 local cache `.codex/sources/kanjidic2/kanjidic2.xml` for Japanese learner-facing stroke counts, on/kun readings, and English definitions.
- Unihan local cache `.codex/sources/Unihan/Unihan_Readings.txt`, `.codex/sources/Unihan/Unihan_DictionaryLikeData.txt`, and `.codex/sources/Unihan/Unihan_IRGSources.txt` for `kVietnamese`, `kDefinition`, and `kTotalStrokes`.
- Existing verified app rows for duplicate kanji (`目`, `律`, `連`, `挙`, `切`, `帯`, `民`) and the local vocabulary-derived examples from lessons 23-25.

| Item | Sources | Change |
|---|---|---|
| `目` | KANJIDIC2 `モク/ボク`, `め/-め/ま-`; existing N3 row; local `一目` | Aligned to `Mục (mắt; mục; điểm)`, replaced word-level `いちもく`, and translated `一目` as `một cái nhìn; thoáng thấy`. |
| `律` | KANJIDIC2 `リツ/リチ/レツ`; existing N3 row; local `一律` | Aligned to `Luật (luật lệ; quy tắc; nhịp điệu)` and translated `一律` as `đồng loạt; thống nhất; như nhau`. |
| `連` | KANJIDIC2 `レン`, `つら.なる/つら.ねる/つ.れる`; existing N3 row; local `一連` | Aligned to `Liên (liên kết; nối liền; liên tục)` and translated `一連` as `một chuỗi; một loạt`. |
| `括` | KANJIDIC2 `カツ`, `くく.る`; Unihan `quát`; local `一括` | Rewrote to `Quát (gộp lại; bao gồm; buộc lại)` and translated `一括` as `gộp chung; xử lý một lượt`. |
| `挙` | KANJIDIC2 `キョ`, `あ.げる/あ.がる/こぞ.る`; existing N2 row; local `一挙に` | Aligned to `Cử (giơ lên; nêu ra; hành động)` and translated `一挙に` as `trong một lần; một lượt`. |
| `切` | KANJIDIC2 `セツ/サイ`, `き.る/...`; existing N2 row; local `一切` | Aligned to `Thiết (cắt; đứt; dứt khoát)` and translated `一切` as `tất cả; hoàn toàn; không hề`. |
| `心` | KANJIDIC2/Unihan `シン`, `こころ`, `tâm`; local `一心` | Rewrote to `Tâm (tim; tâm trí; tấm lòng)` and translated `一心` as `toàn tâm; một lòng`. |
| `帯` | KANJIDIC2 `タイ`, `お.びる/おび`; existing N2 row; local `一帯` | Aligned to `Đới (đai; dải; vùng/đới)` and translated `一帯` as `cả vùng; khu vực`. |
| `敗` | KANJIDIC2/Unihan `ハイ`, `やぶ.れる`, `bại`; local `一敗` | Rewrote to `Bại (thua; thất bại; bị đánh bại)` and translated `一敗` as `một trận thua`. |
| `時` | KANJIDIC2/Unihan `ジ`, `とき/-どき`, `thì`; local `何時` | Rewrote to `Thì (thời gian; lúc; thời điểm)` and translated `何時/いつ` as `khi nào; lúc nào`. |
| `図` | KANJIDIC2 `ズ/ト`, `え/はか.る`; local `意図` | Rewrote to `Đồ (bản đồ; sơ đồ; kế hoạch)` and translated `意図` as `ý định; mục đích`. |
| `営` | KANJIDIC2 stroke `12`, `エイ`, `いとな.む/いとな.み`; local `営む` | Preferred KANJIDIC2 Japanese stroke count over Unihan `11`, rewrote to `Doanh (điều hành; kinh doanh; doanh trại)`, and translated `営む`. |
| `暇` | KANJIDIC2/Unihan `カ`, `ひま/いとま`, `hạ`; local `暇` | Rewrote to `Hạ (thời gian rảnh; nghỉ; nhàn rỗi)` and translated `暇/いとま` as `thời gian rảnh; nghỉ phép`. |
| `動` | KANJIDIC2/Unihan `ドウ`, `うご.く/うご.かす`, `động`; local `異動` | Rewrote to `Động (chuyển động; thay đổi; hành động)` and translated `異動` as `thuyên chuyển; thay đổi vị trí`. |
| `挑` | KANJIDIC2 `チョウ`, `いど.む`; Unihan raw `vẹo`; local `挑む` | Corrected learner-facing Hán-Việt from raw `Vẹo` to `Khiêu`, rewrote to `Khiêu (thách thức; khiêu chiến; đương đầu)`, and translated `挑む`. |
| `稲` | KANJIDIC2 stroke `14`, `トウ/テ`, `いね/いな-`; Unihan definition; local `稲光` | Kept owner-verified `Đạo (lúa; cây lúa)`, preserved the corrected rice-plant meaning, and translated `稲光` as `tia chớp; ánh chớp`. |
| `光` | KANJIDIC2/Unihan `コウ`, `ひか.る/ひかり`, `quang`; local `稲光` | Rewrote to `Quang (ánh sáng; chiếu sáng; tỏa sáng)` and translated `稲光` as `tia chớp; ánh chớp`. |
| `古` | KANJIDIC2/Unihan `コ`, `ふる.い/...`, `cổ`; local `古/いにしえ` | Rewrote to `Cổ (cũ; cổ xưa; thời xưa)` and translated the example as `thời xưa; cổ đại`. |
| `祈` | KANJIDIC2 `キ`, `いの.る`; local `祈り` | Rewrote to `Kỳ (cầu nguyện; khẩn cầu)` and translated `祈り` as `lời cầu nguyện; sự cầu xin`. |
| `鼾` | KANJIDIC2 `カン`, `いびき`; Unihan definition; local `鼾` | Rewrote to `Hãn (tiếng ngáy)`, filled readings/components, and translated the example as `tiếng ngáy`. |
| `今` | KANJIDIC2/Unihan `コン/キン`, `いま`, `kim`; local `今更` | Rewrote to `Kim (nay; hiện tại; bây giờ)` and translated `今更` as `bây giờ mới; đến lúc này`. |
| `更` | KANJIDIC2 `コウ`, `さら/さらに/ふ.ける`; Unihan multi-reading; local `今更` | Normalized learner-facing display to `Canh (thêm nữa; thay đổi; đổi mới)` instead of exposing all raw Hán-Việt variants. |
| `未` | KANJIDIC2 `ミ/ビ`, `いま.だ/ま.だ/ひつじ`; Unihan raw `mùi`; local `未だ` | Corrected learner-facing display from branch-name `Mùi` to `Vị`, rewrote to `Vị (chưa; chưa hoàn thành; tương lai)`, and translated `未だ`. |
| `民` | KANJIDIC2/Unihan `ミン`, `たみ`, `dân`; existing N3 row; local `移民` | Aligned to `Dân (người dân; dân chúng; công dân)` and translated `移民` as `di dân; người di cư/nhập cư`. |

Tagging: all twenty-four entries now carry `vi-source-verified`; old `approved-by-user`, `kanji-metadata-approved`, `unihan-kanji-checked`, and `source-derived-unihan-approved` metadata were removed. No `vi-human-approved` tag was added.

Runtime note: bumped content DB Kanji seed revision from `69` to `70` so existing browsers reseed the changed metadata. No new per-lesson sentinels were added because the revision bump is sufficient for this batch.
...
## Grammar N3 Lessons 1-5 Source-Verified Batch

This batch covers all 20 grammar points in `grammar_n3_1.json` through `grammar_n3_5.json`. Sources consulted for the batch: local N3 scaffold JSON, AmgiDex JLPT N3 grammar list, AI Japanese Tutor JLPT N3 grammar list, MLC Japanese notes where the point appears, and targeted online formation checks from Japanese grammar references for corrected points. Sources were used to verify usage/formation; Vietnamese wording remains original app wording, not copied from external references.

| Lesson | Items | Change |
|---|---|---|
| N3-1 | `ことにする`, `ようにする`, `つもりだ`, `ことにしている` | Checked decision/intention usage and tagged each item `vi-source-verified`. |
| N3-2 | `ことになる`, `ようになる`, `ことになっている`, `ようになっている` | Checked change/result/rule usage and tagged each item `vi-source-verified`. |
| N3-3 | `ために`, `ように`, `代わりに`, `わりに` | Checked purpose/substitution/expectation-gap usage and tagged each item `vi-source-verified`; broadened `代わりに` and `わりに` attachments to verb/plain adjective/noun forms. |
| N3-4 | `うちに`, `たばかり`, `ところだ`, `間に` | Checked time-window/recent-action timing usage and tagged each item `vi-source-verified`; broadened `うちに` and `間に` attachments. |
| N3-5 | `はずだ`, `わけではない`, `わけにはいかない`, `はずがない` | Checked expectation/denial/social-impossibility usage and tagged each item `vi-source-verified`; corrected `わけではない`, added Vない + `わけにはいかない`, and broadened `はず` attachments. |

Runtime note: bumped `GrammarSeeder.kGrammarDataVersion` and content DB grammar seed revision to `17` so existing browsers reseed the changed N3 grammar rows.
...
Live proof after deploy: the existing production browser used normal cache. The rendered N3 Grammar UI showed the corrected row and detail for `代わりに` with `KẾT NỐI: V普通形 / いA / なAな / Nの + 代わりに`, and the corrected row and detail for `わけにはいかない` with `KẾT NỐI: V辞書 / Vない + わけにはいかない`. Page-context fetch with `cache: default` returned `Cache-Control: no-cache` for `main.dart.js`, `grammar_n3_3.json`, and `grammar_n3_5.json`; `sqlite3.wasm` and `drift_worker.js` remained `public, max-age=2592000`. New console warnings/errors after the interaction: `0`.

## Grammar N3 Lessons 6-10 Source-Verified Batch

This batch covers all 20 grammar points in `grammar_n3_6.json` through `grammar_n3_10.json`. Sources consulted for the batch: local N3 scaffold JSON, AmgiDex JLPT N3 grammar list, AI Japanese Tutor JLPT N3 grammar list, MLC Japanese notes where the point appears, and targeted online formation checks from Japanese grammar references for corrected points. Sources were used to verify usage/formation; Vietnamese wording remains original app wording, not copied from external references.

| Lesson | Items | Change |
|---|---|---|
| N3-6 | `すぎる`, `てしまう`, `ことはない`, `かえって` | Checked usage/formation and tagged each item `vi-source-verified`; corrected `すぎる` to `Vます語幹 / いA語幹 / なA + すぎる`, and corrected `かえって` to adverbial `かえって + 文`. |
| N3-7 | `おそれがある`, `に違いない`, `そうにない`, `ないようにする` | Checked risk/certainty/appearance-negative/avoidance usage and tagged each item `vi-source-verified`; added `Vない` to `おそれがある` and clarified adjective/noun attachments for `に違いない`. |
| N3-8 | `ようだ`, `らしい`, `みたいだ`, `ように見える` | Checked conjecture/hearsay/typicality/visual-appearance usage and tagged each item `vi-source-verified`; clarified plain-form, adjective, and noun attachments. |
| N3-9 | `によると`, `によれば`, `そうだ（伝聞）`, `とのことだ` | Checked source-hearsay/reporting usage and tagged each item `vi-source-verified`; clarified `そうだ（伝聞）` attachments and distinguished it from appearance `そうだ`. |
| N3-10 | `によって`, `に対して`, `ていく`, `に応じて` | Checked means/target/change-away/response usage and tagged each item `vi-source-verified`. |

Runtime note: bumped `GrammarSeeder.kGrammarDataVersion` and content DB grammar seed revision to `19` so existing browsers reseed the changed N3 grammar rows. Revision `18` was superseded during live proof because it had already reached the production browser before the final `すぎる`/`かえって` cleanup.
...
## Grammar N3 Lessons 21-25 Source-Verified Batch

This batch covers all 20 grammar points in `grammar_n3_21.json` through `grammar_n3_25.json`. Sources consulted for the batch: local N3 scaffold JSON, AmgiDex JLPT N3 grammar list, AI Japanese Tutor JLPT N3 grammar list, MLC Japanese notes where the point appears, and targeted online formation checks from Bunpro, JLPT Sensei, JapaneseNET, KidNihon, and EasyJapaneseE for corrected points. Sources were used to verify usage/formation; Vietnamese wording remains original app wording, not copied from external references.

| Lesson | Items | Change |
|---|---|---|
| N3-21 | `一方だ`, `つつある`, `につれて`, `に伴って` | Checked one-direction trend/gradual-change/parallel-change/accompanied-change usage and tagged each item `vi-source-verified`; corrected `つつある` to `Vます語幹`. |
| N3-22 | `について`, `に関して`, `に対する`, `をめぐって` | Checked topic/formal-regarding/target/surrounding-debate usage and tagged each item `vi-source-verified`. |
| N3-23 | `にとって`, `として`, `にかけて`, `における` | Checked viewpoint/role/range/formal-location usage and tagged each item `vi-source-verified`; corrected `にかけて` to `N1 + から + N2 + にかけて`. |
| N3-24 | `ほど〜ない`, `というより`, `より〜のほうが`, `に比べて` | Checked negative comparison/refinement/preferred comparison/comparison usage and tagged each item `vi-source-verified`; clarified adjective/noun attachments for `ほど〜ない` and `というより`. |
| N3-25 | `しかない`, `ばかりでなく`, `どんなに〜ても`, `を通して` | Checked no-choice/not-only/no-matter-how/through-medium usage and tagged each item `vi-source-verified`; broadened `ばかりでなく` and `どんなに〜ても` attachments. |

Runtime note: bumped `GrammarSeeder.kGrammarDataVersion` and content DB grammar seed revision to `22` so existing browsers reseed the changed N3 grammar rows. This completes the current N3 grammar pass.
...
## Grammar N4 Lessons 36-40 Source-Verified Batch

This batch covers all 20 grammar points in `grammar_n4_36.json` through `grammar_n4_40.json`. Sources consulted for the batch: local N4 curriculum JSON/examples, JLPT Mastery N4 grammar category list, AI Japanese Tutor and JLPT Sensei notes for `ように`, passive, nominalization, `ので`, `ために`, `おかげで`, embedded questions, `かどうか`, and `てみる`, plus cross-checks against the app's N4 examples. Sources were used to verify usage/formation; Vietnamese wording remains original app wording, not copied from external references.

| Lesson | Items | Change |
|---|---|---|
| N4-36 | `ように`, `ようになりました`, `ようにしています`, `ようにしてください` | Replaced old `vi-human-approved` with `vi-source-verified`; clarified dictionary/negative and potential attachments. |
| N4-37 | Passive form and passive sentence patterns | Replaced old approval tag with source verification; clarified `受身形` and passive verb structures. |
| N4-38 | Nominalizing `の`, evaluation, forgetting, and `こと` | Replaced old approval tag with source verification; corrected nominalization to `V辞書 + の...` and `V辞書 + こと`. |
| N4-39 | Cause/reason `て`, `ので`, `ために`, `おかげで` | Replaced old approval tag with source verification; clarified verb, i-adjective, na-adjective, and noun attachments. |
| N4-40 | Embedded questions, `かどうか`, `てみます` | Replaced old approval tag with source verification; corrected `かどうか` and question-word embedded forms to distinguish na-adjective stems and nouns. |

Runtime note: bumped `GrammarSeeder.kGrammarDataVersion` and content DB grammar seed revision to `26` so existing browsers reseed the changed N4 grammar rows.
