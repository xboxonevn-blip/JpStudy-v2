...
- Kanji expansion status update: QA-B-002 now has a reproducible KANJIDIC2 old-JLPT coverage audit. Baseline current unique Kanji is `638` vs KANJIDIC2 old-JLPT unique `2230`; missing source Kanji are N5 `33`, N4 `157`, N2 `654`, N1 `1168`. KANJIDIC2 has no modern N3 tier, so a separate modern JLPT level map is required before generation. N3, N2, and current N1 source verification are complete with incomplete current entries at `0`; N1 lessons 23-25 closed the remaining `24` incomplete entries by fixing readings, related kanji, and learner-facing Hán-Việt conflicts such as `挑 -> Khiêu` and `未 -> Vị`. Latest live proof rendered the real VI/N1 Kanji detail modals for `挑` and `未` with normal browser cache; page-context fetch returned content/shell `Cache-Control: no-cache`; new console warnings/errors were `0`. QA-A-017 restored `no-cache` for app-shell/content assets; deploy verification includes normal-cache header proof and rendered UI proof, with CDP cache-disabled reserved only as a diagnostic control. Remaining QA-B-002 debt is expansion beyond the current 638 authored kanji, not completeness of current entries.
